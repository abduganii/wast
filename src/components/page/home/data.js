import { Person, Photo, Reflesh } from "../../UI/icon";

export const Info = [
    {
        id: 1,
        icon:()=> Person(),
        text:"Более 80% уборщиков ораганизаций - женщины",
    },
    {
        id: 2,
        icon:()=> Photo(),
        text:"Средняя зарплата уборщика в Ташкенте - 2-3 млн сум",
    },
    {
        id: 3,
        icon: ()=>Reflesh(),
        text:"Только 26% отходов в Узбекситане подвергаются на данный момент переработке",
    }
]
export const ServiceArray = [
    {
        id: 1,
        number:1,
        text:"You sort waste (plastic, paper, glass)",
    },
    {
        id: 2,
        number:2,
        text:"Submit your application through the application",
    },
    {
        id: 3,
        number:3,
        text:"The courier arrives at the appointed time",
    },
    {
        id: 4,
        number:4,
        text:"You get additional income, and waste - new life!",
    },
    {
        id: 5,
        number:5,
        text:"The courier takes the waste for recycling and closes the application",
    }
]
export const BlogsArray = [
    {
        id: 1,
        image:'/image/blog.png',
        title:"Leverage hiring forward",
        text:"You sort waste (plastic, paper, glass)",
    },
    {
        id: 2,
        image:'/image/blog.png',
        title:"Leverage hiring forward",
        text:"Submit your application through the application",
    },
    {
        id: 3,
        image:'/image/blog.png',
        title:"Leverage hiring forward",
        text:"The courier arrives at the appointed time",
    },
    {
        id: 4,
        image:'/image/blog.png',
        title:"Leverage hiring forward",
        text:"You get additional income, and waste - new life!",
    }
]


export const CertificateArray = [
    {
        id: 1,
        image:'/image/sertificate.png',
        title:"Sertificate name",
    },
    {
        id: 2,
        image:'/image/sertificate1.png',
        title:"Sertificate name",
    },
    {
        id: 3,
        image:'/image/sertificate2.png',
        title:"Sertificate name",
    },
    {
        id: 4,
        image:'/image/sertificate3.png',
        title:"Sertificate name",
    }
]
